...
class HW_PWM:
    ...
    def set_duty_cycle(self, duty_cycle_percent):
        ...
        if duty_cycle_percent >= 100:
            ...
            self.duty_cycle = 100
        if duty_cycle_percent > 0 and duty_cycle_percent < 100:
            self.duty_cycle_percent = duty_cycle_percent
            self.duty_cycle = int(self.period * (self.duty_cycle_percent / 100))

        duty_cycle_cmd = "echo " + str(self.duty_cycle) + " > " + PWM_PATH + "/pwm0/duty_cycle"
        print(f"Setting duty cycle to {self.duty_cycle_percent}% ")
        os.system(duty_cycle_cmd)
        time.sleep(0.5)
