...
class HW_PWM:
    ...
    def set_duty_cycle(self, duty_cycle_percent):
        ...
        duty_cycle_percent = (duty_cycle_percent / 100.0) * self.period
        duty_cycle_cmd = "echo " +str(int(duty_cycle_percent)) + " > " + PWM_PATH + "/pwm0/duty_cycle"
        print(f'{duty_cycle_percent}% , {self.duty_cycle} ns')
        os.system(duty_cycle_cmd)
        time.sleep(0.5)
